...
describe Address::Importer do
  ...
  it 'fetches and updates addresses' do
    ...
    bs_bern = Address.find_by(street: 'Belpstrasse', zip_code: 3007)
    expect(bs_bern.street).to eq 'Belpstrasse'
    expect(bs_bern.town).to eq 'Bern'
    expect(bs_bern.zip_code).to eq 3007
    expect(bs_bern.numbers).to eq [36,37,38,40]

    bs_muri = Address.find_by(street: 'Belpstrasse', zip_code: 3074)
    expect(bs_muri.street).to eq 'Belpstrasse'
